...
function findAndReplaceProps(j: JSCodeshift,root: Collection,tagName: string) {
  ...
  root
    ...
    .forEach((el) => {
      ...
      type styleExpPropsType = namedTypes.Property | namedTypes.ObjectProperty | namedTypes.SpreadElement | namedTypes.SpreadProperty | namedTypes.ObjectMethod
      let styleExpProps:styleExpPropsType[] =[]
      ...
      const attributes = el.node.openingElement.attributes?.filter((a) => {
        ...
        if (a.name.name === 'style') {
          if (a.value?.type === 'JSXExpressionContainer' &&a.value.expression.type === 'ObjectExpression') {
            styleExpProps = a.value.expression.properties
          } 
          else if (a.value?.type === 'JSXExpressionContainer' && a.value.expression.type === 'Identifier') {
            styleExpProps = [
